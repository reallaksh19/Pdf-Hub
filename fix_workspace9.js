...
// Remove definitions safely using regex instead of substring matches that might break braces
content = content.replace(/const BoxNode: React\.FC<BoxNodeProps> = \(\{[\s\S]*?\}\);/g, '');
content = content.replace(/const CalloutNode: React\.FC<CalloutNodeProps> = \(\{[\s\S]*?\}\);/g, '');
content = content.replace(/const LineLikeNode: React\.FC<\{[\s\S]*?\}\);/g, '');
content = content.replace(/interface BoxNodeProps \{[\s\S]*?\}/g, '');
content = content.replace(/interface CalloutNodeProps \{[\s\S]*?\}/g, '');
content = content.replace(/function arrowHeadPolygon\([\s\S]*?\}/g, '');

content = content.replace(/const \[editingId, setEditingId\] = React\.useState<string \| null>\(null\);/g, '');
content = content.replace(/const \[editingValue, setEditingValue\] = React\.useState\(''\);/g, '');

content = content.replace(/const commitTextEdit = \([\s\S]*?\}\);/g, '');
content = content.replace(/const startTransform = \([\s\S]*?\}\);/g, '');
content = content.replace(/const startAnchorDrag = \([\s\S]*?\}\);/g, '');
