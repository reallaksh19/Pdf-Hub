...
## Goal
Deliver production-safe implementation for review index, statuses/filters, thread metadata and summary export, integrated with existing architecture and with no duplicate mutation execution path.

## Owned write scope
- review index, statuses/filters, thread metadata and summary export
...
- Updated docs under Docs/execution/30_evidence/A7/RESULT.md.
...
export interface ReviewThread {
  ...
  author: string;
  createdAt: number;
  replies: Array<{ id: string; author: string; body: string; createdAt: number }>;
