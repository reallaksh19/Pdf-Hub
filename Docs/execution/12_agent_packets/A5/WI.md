...
## Goal
Deliver production-safe implementation for pointer event matrix for select/text-mark/draw/callout modes, integrated with existing architecture and with no duplicate mutation execution path.

## Owned write scope
- pointer event matrix for select/text-mark/draw/callout modes
...
- Updated docs under Docs/execution/30_evidence/A5/RESULT.md.
- Regression-safe integration against current ribbon/sidebar/workspace flows.

## Critical code snippets (target shape)
```ts
const TOOL_BEHAVIOR: Record<ActiveTool, {
  textSelection: boolean;
  objectSelection: boolean;
  marquee: boolean;
  canvasDraw: boolean;
}> = {
  select: { textSelection: true, objectSelection: true, marquee: true, canvasDraw: false },
  highlight: { textSelection: true, objectSelection: false, marquee: false, canvasDraw: false },
  line: { textSelection: false, objectSelection: false, marquee: false, canvasDraw: true },
};
```
