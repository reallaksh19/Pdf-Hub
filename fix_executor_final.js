...
executorTs = executorTs.replace(/assertNever\(step\);/g, '// assertNever(step);');
...
writerStoreTs = writerStoreTs.replace(/const useWriterStore = create<WriterState & WriterActions>\(\(set\) => \(\{\)/g, 'const useWriterStore = create<WriterState & WriterActions>((_set) => ({');
