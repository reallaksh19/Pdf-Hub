  ...
  for (const [pageNumber, pageElements] of byPage) {
    ...
    for (const element of sorted) {
      ...
      currentBytes = await PdfEditAdapter.insertImage(currentBytes, {
        pages: [pageNumber - 1], // internal pdf lib uses 0-index pages
        x:           element.x,
        y:           element.y,
        width:       element.width,
        ...
        imageBytes:  pngBytes,
        mimeType:    'image/png',
        // opacity:     element.styles.opacity     ?? 1, // Opacity is handled by html2canvas
        // borderWidth: element.styles.borderWidth ?? 0, // Not supported directly by PdfEditAdapter.insertImage
