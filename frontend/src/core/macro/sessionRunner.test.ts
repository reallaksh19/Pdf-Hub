...
describe('runMacroRecipeAgainstSession', () => {
  ...
  it('updates session for normal runs', async () => {
    await runMacroRecipeAgainstSession({ id: 'r2', name: 'R2', steps: [] });

    expect(useSessionStore.getState().pageCount).toEqual(3);
    expect(useSessionStore.getState().selectedPages).toEqual([2]);
  });

  it('skips session update for dry runs', async () => {
    await runMacroRecipeAgainstSession({ id: 'r3', name: 'R3', steps: [] }, { dryRun: true });

    expect(useSessionStore.getState().selectedPages).toEqual([1, 2]);
  });
