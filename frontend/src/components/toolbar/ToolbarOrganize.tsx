      ...
      <div className="flex items-center space-x-1">
        ...
        <Tooltip content={
          activeIndices.length > 0
            ? `Extract ${activeIndices.length} page(s) — copies to a new PDF, this document is unchanged`
        ...
        <Tooltip content={
          activeIndices.length > 0
            ? `Split ${activeIndices.length} page(s) — moves to new PDF and removes from this document`
            : 'Split — select pages in the thumbnail panel first'
        }>
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8 hover:text-red-500 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-950 transition-colors"
            onClick={handleSplitOut}
            disabled={!workingBytes || activeIndices.length === 0}
          >
