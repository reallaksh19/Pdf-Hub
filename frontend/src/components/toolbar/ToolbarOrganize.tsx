...
export const ToolbarOrganize: React.FC = () => {
  ...
  const handleSplit = async () => {
    ...
    const pageCountSplit = activePages.length || 1;
    const confirmed = window.confirm(
      `Split will permanently remove ${pageCountSplit} page(s) from this document and save them to a new PDF.\n\nThis cannot be undone. Continue?`
    );
    if (!confirmed) return;
    ...
    <>
      ...
      <div className="flex items-center space-x-1">
        ...
        <Tooltip content="Split — moves selected pages to a new PDF and removes them from this document.">
          <Button variant="ghost" size="icon" className="h-8 w-8 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950" onClick={handleSplit} disabled={!workingBytes}>
