      ...
      >
        {pageAnnotations
          ...
          .map(annotation => {
            ...
            const anchor = readAnchor(annotation, draftAnchors[annotation.id] ?? null);
            const points = draftLinePoints[annotation.id];

            let NodeComponent: React.FC<AnnotationNodeProps<any>>;
            ...
            return (
              <NodeComponent
                ...
                onTransformStart={(event, mode) => startTransform(event, annotation, mode)}
                onDoubleClick={() => {
                  if (annotation.data?.locked === true) return;
                ...
                onCommitEdit={(v) => commitTextEdit(annotation, v)}
                points={points}
                {...(annotation.type === 'callout' ? {
                  anchorPoint: anchor,
                  onAnchorDragStart: (event: React.PointerEvent) => startAnchorDrag(event, annotation),
                } : {})}
              />
            );
        })}
